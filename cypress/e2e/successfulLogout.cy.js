describe('Successful Logout', function () {
    context('Existing User', () => {
      it(`Navigating to the login page`, () => {
        ...
        // Hover over the avatar element
        cy.get('.uni-avatar__content-container').trigger('mouseover').should('be.visible');
        // Click on the "Log Out" button
        cy.get('.hui-globalusermenu__item[data-qa-id="webnav-usermenu-logout"]').each(($element) => {
        cy.wrap($element).click({ force: true });
        cy.url().should('eq', 'https://www.hudl.com/en_gb/');
        });
      });
    });
  });